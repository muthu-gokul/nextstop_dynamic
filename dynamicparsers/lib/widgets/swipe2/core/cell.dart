...
class SwipeActionCell extends StatefulWidget {
  ...
  final Function(int) swipeCallBack;
  final Function(int) closeCallBack;
...
class SwipeActionCellState extends State<SwipeActionCell>
  ...
  void _onHorizontalDragEnd(DragEndDetails details) async {
    ...
    if (lastItemOut && widget.performsFirstActionWithFullSwipe) {
      ...
      if (whenTrailingActionShowing && widget.trailingActions != null) {
        await widget.trailingActions?[0].onTap.call(completionHandler,widget.trailingActions?[0].index??-1);
      } else if (whenLeadingActionShowing && widget.leadingActions != null) {
        await widget.leadingActions?[0].onTap.call(completionHandler,widget.index??-1);
      }
    } else {
  ...
  void _open({required bool trailing, bool animated = true}) {
    // print("ggggfgfgfgjhgjfbg OPEN ${widget.index}");
    // Provider.of<BillingNotifier>(context,listen:false).updateSlideItemIndex(widget.index??-1);
    widget.swipeCallBack(widget.index??-1);
    if (animated) {
      _resetAnimValue();
  ...
  Future<void> closeWithAnim() async {
    // print("closeee ${widget.index}");
    // Provider.of<BillingNotifier>(context,listen:false).updateSlideItemIndex(-1);
    widget.closeCallBack(widget.index??-1);
...
class SwipeAction {
  ...
  ///onTap callback
  ///点击事件回调
  final Function(CompletionHandler,int) onTap;
